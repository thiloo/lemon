Meteor.methods({
    'solidity.generate.contract': function(template) {
        ...
        const sourceCode3 = `pragma solidity ^0.4.4; contract SimpleStorage { string storedData; address producer; mapping (address => uint) public coinBalanceOf; event CoinTransfer(address sender, address receiver, uint amount); function SimpleStorage(string x, uint supply) { storedData = x; producer = msg.sender; coinBalanceOf[msg.sender] = supply; } function get() constant returns (string x) { return storedData; } function getProducer() constant returns (address y) { return producer; } function sendCoin(address receiver, uint amount) returns(bool sufficient) { if (coinBalanceOf[msg.sender] < amount) return false; coinBalanceOf[msg.sender] -= amount; coinBalanceOf[receiver] += amount; CoinTransfer(msg.sender, receiver, amount); return true; } }`;
